...
void executeTests(const std::string& _name, std::pair<boost::filesystem::path const&, AccessSwitch> _testPathAppendix, boost::filesystem::path const& _fillerPathAppendix, std::function<json_spirit::mValue(json_spirit::mValue const&, bool)> doTests);
